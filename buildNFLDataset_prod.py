...
for team in teamsList:
    ...
    tmp["rolling10GamePoints"] = tmp["points"].rolling(10).mean()
    tmp["rolling10GamePointsAgainst"] = tmp["pointsAgainst"].rolling(10).mean()
    tmp["rolling10Record"] = tmp["isWinner"].rolling(10).mean()
    tmp["rolling5Record"] = tmp["isWinner"].rolling(5).mean()
    tmp["rolling10Rushing"] = tmp["rushingYards"].rolling(10).mean()
    tmp["rolling10Passing"] = tmp["passingYards"].rolling(10).mean()
    tmp["rolling10RushingAgainst"] = tmp["rushingYardsAgainst"].rolling(10).mean()
    tmp["rolling10PassingAgainst"] = tmp["passingYardsAgainst"].rolling(10).mean()
...
# %%
finalStats.to_csv("prodDataset.csv")
